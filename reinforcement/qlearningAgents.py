...
#    return action
    ...
    for action in legalActions:
    	expoente = self.getQValue(state, action)/temperatura
        soma += np.exp(expoente)
    for action in legalActions:
        bestaction[action] = np.exp(self.getQValue(state, action)/temperatura)/soma
    
    maxProb = bestaction.argMax()
    if maxProb:
         action = maxProb
